    ...
    def spec(self) -> ToolSpec:
        return ToolSpec(
            ...
            capability=ToolCapability(
                namespace="output.delivery",
                actions=("deliver",),
                entity_types=("user_message", "artifact_attachment"),
                input_modes=("text", "artifact_path"),
                output_modes=("user_message",),
                limitations=("Attachment existence is checked, but task-level output-contract validity is not.",),
            ),
            execution=ToolExecutionPolicy(
                side_effect="user_visible_output",
                concurrency_pool="tool",
                supports_parallel=False,
                idempotent=False,
            ),
            dependencies=ToolDependencies(services=("runtime_outbox",)),
            preflight_checks=("tool_authorized", "attachments_exist", "attachments_in_workspace"),
        )
