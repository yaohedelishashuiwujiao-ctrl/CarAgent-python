...
from src.dashboard import DashboardState, DashboardManager, make_dashboard_callbacks
...
class ClawdREPL:
    ...
    def __init__(self, provider_name: str = "glm", stream: bool = False):
        ...
        # Dashboard for split-screen visualization
        self.dashboard_state = DashboardState()
        self.dashboard_manager = DashboardManager(self.console, self.dashboard_state)
        self.dashboard_enabled = True  # enabled by default
    ...
    def _print_startup_header(self):
        ...
        footer = Text("/help  •  /tools  •  /stream  •  /dashboard  •  /exit", style="dim")
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # Dashboard: pause Live for prompt_toolkit input
                if self.dashboard_enabled and self.dashboard_manager is not None:
                    self.dashboard_manager.pause_for_input()
                ...
                # Dashboard: resume after input
                if self.dashboard_enabled and self.dashboard_manager is not None:
                    self.dashboard_manager.resume_after_input()
    ...
    def handle_command(self, command: str):
        ...
        elif cmd == '/dashboard' or cmd.startswith('/dashboard '):
            self.dashboard_enabled = not self.dashboard_enabled
            status = "enabled" if self.dashboard_enabled else "disabled"
            self.console.print(f"[green]Dashboard {status}.[/green]")
...
- `/dashboard` - Toggle split-screen dashboard mode
...
"""
    ...
    def chat(self, user_input: str, max_turns: int = 20):
        ...
        # Dashboard: append user message to conversation lines
        if self.dashboard_enabled and self.dashboard_state is not None:
            self.dashboard_state.conversation_lines.append(f"\n[bold blue]❯ {user_input}[/bold blue]\n")
            self.dashboard_state.begin_turn()

        try:
            # Dashboard mode: skip "Assistant" header and spinner
            if self.dashboard_enabled and self.dashboard_state is not None:
                self._run_chat_with_dashboard(user_input, max_turns)
            else:
                self._run_chat_linear(user_input, max_turns)
    ...
    def _run_chat_linear(self, user_input: str, max_turns: int) -> None:
        """Original linear chat flow (non-dashboard mode)."""
        self.console.print("\n[bold]Assistant[/bold]")
        ...
        if result.usage:
            ...
            if input_tokens > 0 or output_tokens > 0:
                ...
                self.cost_tracker.record_tokens(input_tokens, output_tokens, result.num_turns)
    ...
    def _run_chat_with_dashboard(self, user_input: str, max_turns: int) -> None:
        """Dashboard mode: use split-screen rendering with data flow visualization."""
        assert self.dashboard_state is not None
        assert self.dashboard_manager is not None

        # Build dashboard-wrapped callbacks
        dashboard_on_event, dashboard_on_text = make_dashboard_callbacks(
            self.dashboard_state,
            self.dashboard_manager,
        )

        # Run agent loop with dashboard callbacks
        # No spinner - dashboard handles status display
        result = run_agent_loop(
            conversation=self.session.conversation,
            provider=self.provider,
            tool_registry=self.tool_registry,
            tool_context=self.tool_context,
            max_turns=max_turns,
            stream=False,  # Dashboard handles output display
            verbose=False,
            on_event=dashboard_on_event,
            on_text_chunk=dashboard_on_text,
        )
        ...
        if result.usage:
            ...
            self.dashboard_state.complete_phase("LLM_CALL")
            self.dashboard_state.complete_phase("TOOL_DISPATCH")
            self.dashboard_state.complete_phase("TOOL_RESULT")
            self.dashboard_state.complete_phase("RESPONSE")
        ...
        if result.response_text and result.response_text != "[Max tool turns reached]":
            self.dashboard_state.conversation_lines.append(f"\n{result.response_text}\n")
        else:
            self.dashboard_state.conversation_lines.append(f"\n[dim]{result.response_text}[/dim]\n")

        # End turn and update
        tool_count = len([e for e in self.dashboard_state.events if e["kind"] == "tool_use"])
        usage = result.usage or {}
        self.dashboard_state.end_turn(
            usage.get("input_tokens", 0),
            usage.get("output_tokens", 0),
            tool_count=tool_count,
        )

        # Final dashboard update - render the completed state
        self.dashboard_manager.update()
