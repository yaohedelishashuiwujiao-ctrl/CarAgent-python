...
def _get_default_config_from_providers() -> dict[str, Any]:
    ...
    return {
        "default_provider": os.getenv("CLAWD_DEFAULT_PROVIDER", "anthropic"),
        "providers": {
            name: {
                "api_key": "",
...
def get_provider_config(provider: str) -> dict[str, Any]:
    ...
    if not provider_config.get("api_key"):
        ...
        if provider in {"ark", "ark_responses"}:
            ...
            env_key = os.getenv("GLM_API_KEY", "").strip() or os.getenv("ZHIPUAI_API_KEY", "").strip()
...
def get_default_provider() -> str:
    ...
    """
    env_provider = os.getenv("CLAWD_DEFAULT_PROVIDER", "").strip()
    if env_provider:
        return env_provider
    config = load_config()
