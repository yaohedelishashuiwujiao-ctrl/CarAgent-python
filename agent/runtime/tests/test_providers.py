...
from src.providers.ark_responses_provider import ArkResponsesProvider
...
class TestArkResponsesProvider(unittest.TestCase):
    def test_tool_choice_capabilities_exclude_unsupported_required_mode(self):
        provider = ArkResponsesProvider(api_key="test_key")

        self.assertIsNone(provider.format_tool_choice("required"))
        self.assertIsNone(provider.format_tool_choice("auto"))
        self.assertIsNone(provider.format_tool_choice("specific", "SubjectsSqlSchema"))
