...
这层解决的是产物型 Agent 最常见的假完成问题。

文本问答里，模型输出一段答案就可能算完成。但产物任务不一样，用户要的是文件、图表或结构化输出。模型很容易说：

```text
我已经为你生成了 PPT，内容如下...
```

但实际没有文件。OutputContract 把这种任务从“语言完成”改成“产物完成”。
...
这就是为什么 OutputContract 必须放在 Runtime，而不能放在 prompt 里。

Prompt 只能提醒模型“要生成文件”，但 Runtime 可以实际检查文件系统和工具输出。一个是软约束，一个是硬验收。
