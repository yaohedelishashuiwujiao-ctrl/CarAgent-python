...
这套材料不是代码总结。正确读法是先抓住系统为什么要这样设计：

```text
业务问题：汽车知识、结构化参数、图表/PPT、长任务、多用户并发混在一起
简单方案：一次 RAG 检索 + 一次 LLM 回答
问题暴露：不能选择工具，不能验证产物，不能控制成本，不能解释失败
工程方案：Agent Loop + Tool System + RAG Tool + Scheduler + State + Contract
源码作用：证明这些机制真实存在，并帮助面试时回答细节追问
```
...
第一遍：只看每篇的“这一层解决什么问题”和图，先建立全局链路。不要先背源码路径。

第二遍：看“为什么这么做”和“流程怎么跑”，理解它解决了什么工程矛盾。

第三遍：再看“我们项目里的真实源码”，把概念对应到代码文件。

第四遍：看“面试官可能怎么问”，按 30 秒回答、2 分钟展开、源码级细节训练。
