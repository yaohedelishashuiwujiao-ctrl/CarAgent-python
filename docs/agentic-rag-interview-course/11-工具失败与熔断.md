...
熔断不是为了让 Agent 更快失败，而是为了防止它在错误方向上持续消耗资源。
...
|---|---|
| SQL 参数写错 | 允许模型修正一次 |
| 知识库无数据 | 换 query、扩大检索或给 coverage boundary |
| 权限拒绝 | 不应重试，应该说明权限边界 |
| 依赖不可用 | 禁用该工具或 fallback |
| 产物 contract 未满足 | 不能用文本假装完成 |

所以熔断不是一个简单计数器，而是 Runtime 根据 outcome_status、进展、重复情况和 contract 状态做恢复决策。
...
| 概念 | 含义 |
|---|---|
| failure | 工具调用失败或返回低质量状态 |
| no progress | 这次动作没有让任务更接近完成 |

有些结果虽然不是 error，但仍然是 no progress，比如重复检索结果、coverage insufficient、empty result。RunBudget 和 RunState 就是为了把这类“没报错但没推进”的情况记录下来。
