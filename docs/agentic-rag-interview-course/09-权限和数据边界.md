...
模型可以理解“不要访问越权数据”这句话，但它不能成为安全边界。真正的边界必须由后端和 Runtime 强制执行：

```text
Backend 创建 job 时固化用户身份、角色、data_scope、allowed_tools
Runtime 执行工具时只读取这份快照
工具内部再做细粒度校验
```
...
| 问题 | 设计选择 |
|---|---|
| 模型可能幻觉或忽略权限规则 | Runtime 强制 allowed_tools / data_scope |
| 用户权限可能在任务执行中变化 | job 入队时做权限快照，保证本次执行可审计 |
