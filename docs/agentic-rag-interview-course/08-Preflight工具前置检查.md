...
这里的核心原则是：模型输出的是“意图”，不是“授权执行”。

如果没有 Preflight，Agent 会把模型每个 tool_use 都当成可信命令，这在生产环境里很危险：

| 风险 | 例子 |
|---|---|
| 越权访问 | SQL 工具查到用户 data_scope 之外的数据 |
| 资源浪费 | 参数明显缺失还进入数据库或 RAG 检索 |
| 危险副作用 | 产物生成、写文件、外部请求不受限制 |
| 错误扩大 | 依赖已经 unhealthy，还继续调用 |

Preflight 的设计是把“能不能做”放在“怎么做”之前。这样无效动作不会进入真正资源池，拒绝本身也会作为 observation 回给模型。
...
这不是简单的错误拦截，而是一种可恢复控制。

Preflight 拒绝后，系统通常还有几种选择：
...
因此 Preflight 的输出也必须是结构化的，而不是直接抛异常。
