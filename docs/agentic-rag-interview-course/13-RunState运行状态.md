...
这里的关键不是“保存历史消息”。历史消息是给模型看的，RunState 是给 Runtime 做控制判断的。

如果只依赖 messages，有两个问题：

| 问题 | 后果 |
|---|---|
| 上下文越来越长 | 模型可能忽略早期失败和证据 |
| 状态不可计算 | Runtime 不知道是否停滞、是否重复、是否已有足够证据 |

RunState 把运行过程压成结构化账本：
...
所以 RunState 是 Agent Loop 的“运行期控制状态”，不是普通聊天记忆。
...
RunState 的设计目标是让 Runtime 能回答三个问题：

1. 这次动作有没有让任务更接近完成？
2. 模型是不是在重复走失败路径？
3. 当前证据和计划是否足够支撑最终回答？

这三个问题回答不了，Agent 就只能靠模型主观判断什么时候停。
