...
TodoWrite 的价值不是让模型写一个漂亮清单，而是把隐式推理外显成可检查状态。

复杂任务里，模型脑子里可能有计划，但 Runtime 看不见。看不见就有三个问题：

| 问题 | 后果 |
|---|---|
| 不知道当前做到哪一步 | 无法判断是否停滞 |
| 不知道模型有没有漏步骤 | 例如 PPT 少了取数或 citation |
| 不知道“完成”是否有证据 | 模型可能把未执行步骤标成完成 |

TodoWrite 把计划变成结构化状态，Runtime 可以记录 plan_revision、active_step 和 progress_events。
...
这里也有一个重要取舍：我们没有把复杂任务写死成固定 DAG。

原因是汽车 Agent 的任务跨度很大：有的要查参数，有的要查手册，有的要对比市场，有的要生成 PPT。固定 DAG 会很僵硬。TodoWrite 是中间方案：
